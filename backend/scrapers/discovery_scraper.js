...
async function intelligentSearchAndCollect(page, siteUrl, searchTerm) {
  ...
  const searchBox = await page
    .locator(
      'input[type="search"], input[placeholder*="search" i], input[placeholder*="keyword" i], input[aria-label*="search" i], input[role="combobox"], input[data-automation-id*="search"], input[class*="search"], input[title*="search"], input[type="text"]'
    )
    .first();
...
(async () => {
  ...
  while (true) {
    let cycleTotal = 0;
    console.log(`\n🔁 New discovery cycle starting at ${new Date().toISOString()}`);

    for (const site of siteUrls) {
      try {
        cycleTotal += await intelligentSearchAndCollect(page, site, searchTerm);
      } catch (e) {
    ...
    console.log(`\n✅ Discovery cycle complete: ${cycleTotal} new jobs added.`);
