...
async function scrapeSite(page, siteUrl, role) {
    // Smart URL construction based on existing parameters
    const url = new URL(siteUrl);
    
    // Common search parameter names to look for
    const searchParams = ['keyword', 'keywords', 'q', 'query', 'search'];
    let searchParamFound = false;
    
    // Check if URL already has a search parameter
    for (const param of searchParams) {
        if (url.searchParams.has(param)) {
            url.searchParams.set(param, role);
            searchParamFound = true;
            break;
        }
    }
    
    // If no search param found, add 'keywords' as default
    if (!searchParamFound) {
        url.searchParams.append('keywords', role);
    }
    ...
    try {
        ...
        log(`   ✅ Extracted ${jobs.length} job links`);
        
        if (jobs.length > 0) {
            log(`   📋 Sample jobs:`);
            jobs.slice(0, 3).forEach(j => log(`     • ${j.title.substring(0, 50)}`));
        }
...
async function processJobBatch(jobs, siteName) {
    ...
    for (const job of jobs) {
        // No role/keyword filtering; only de-duplicate by URL
...
async function mainLoop() {
    ...
    log(`🚀 Job Scraper Started with Site-Specific Logic`);
    ...
    while (isRunning) {
        ...
        for (const role of currentRoles) {
            ...
            for (const siteUrl of sites) {
                ...
                log(`   📊 Added: ${result.addedCount} | Skipped: ${result.duplicateCount}\n`);
                log(`😴 ${SNOOZE_BETWEEN_SITES/1000}s pause\n`);
                
                await new Promise(resolve => setTimeout(resolve, SNOOZE_BETWEEN_SITES));
            }
        }
        
        log(`✅ Full cycle complete - waiting 5min\n\n`);
