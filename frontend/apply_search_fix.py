...
# 2) Add chip management before FETCH JOBS
...
# 3) Replace the ACTUAL applyFilters that exists (with status filter)
old = """function applyFilters() {
    filteredJobs = allJobs.filter(job => selectedStatuses.includes(job.status));
    displayJobs();
}"""
...
p.write_text(t)
print("✅ Applied search with correct status filter replacement")
